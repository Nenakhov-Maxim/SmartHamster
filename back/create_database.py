...
def _load_data(session: Session):
    ...
    statis_gr1 = Status(group_name='Выполнено')
    statis_gr2 = Status(group_name='Выполняется')
    statis_gr3 = Status(group_name='На контроле')
    statis_gr4 = Status(group_name='Контролируется')
    session.add(group1)
    session.add(group2)
    session.add(group3)
    session.add(group4)
